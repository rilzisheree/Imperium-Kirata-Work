...
-- No server-side stat changes; the server tracks state for cleanup and runs
-- the private IM loop that fires real IM messages to the affected player
-- (same path as the `im` command: CommandRemotes.IM:FireClient).
local heartbeatData = {}
...
local MUSIC_TRACKS = {
	Calm = {
		...
		"89891313789297","110395238223034","95497960463962","129128626355798",
	},
	Intense = {
...
local COLOUR_NAMES = {
	red=true, darkred=true, blue=true, green=true, yellow=true, orange=true,
...
HANDLERS["volume"] = function(executor, args)
	...
	if not raw then
		fail(executor, "Usage: volume <0-100>")
		return
	end
	...
	local volume = VolumeManager.setVolume(executor.UserId, raw)
...
local function applyHeartbeat(target: Player, duration: number): (boolean, string)
	...
	if data then
		...
		-- then restart both the timer and the IM loop for the new duration.
		data.token += 1
		...
		-- Restart the IM loop with the refreshed token and duration.
...
-- parses a trailing "true"/"false" argument; returns nil if it isn't one of those two
local function parseBool(raw: string?): boolean?
...
-- parses a positive-integer asset ID; returns nil if invalid
