...
local CosmeticsManager  = require(script.Parent:WaitForChild("CosmeticsManager")    :: ModuleScript)
...
for _, player in Players:GetPlayers() do
        ...
        -- reapply permanently saved cosmetics for anyone already in the server
        player.CharacterAdded:Connect(function(character)
                CosmeticsManager.onCharacterAdded(player, character)
        end)
...
HANDLERS["sethair"] = function(executor, args)
        ...
        if #failures > 0 then
                msg = msg .. " (" .. #failures .. " failed: " .. table.concat(failures, "; ") .. ")"
        end
        ok(executor, msg)
end
