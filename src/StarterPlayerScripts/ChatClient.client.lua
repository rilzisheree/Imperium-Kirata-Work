--[[
        ...
        Proximity chat system:
          • Disables default Roblox chat
          • Input bar (press / to focus)
          • Chat bubbles pinned above heads via WorldToViewportPoint (zoom-proof)
          • Real-time distance tiers: full (≤23 studs) / [Inaudible] (≤33) / hidden
          • Chat logs window (open with `chatlogs` command)
--]]
...
local FADE_IN           = 0.2
local FADE_OUT          = 0.5
local WORLD_Y_OFFSET    = 1.5   -- studs above Head center
local FULL_DISTANCE     = 23    -- studs: full message
...
do
        local c = Instance.new("UICorner", inputFrame)
...
do
        local c = Instance.new("UICorner", sendBtn)
...
-- RenderStepped: move bubble containers + update text per distance
...
-- Search box — starts non-editable; enabled only when window is open
...
searchBox.TextEditable       = false   -- prevents focus-stealing while window is closed
...
-- Expose for CommandBar via the shared table
...
do
        ...
        -- Invisible button covering title bar (excluding close button)
