...
local VIG_BEAT_TRANS  = 0.52   -- strong beat flash (lower = more red)
...
local BEAT_REST_MIN      = 0.60   -- seconds between full cycles (lub-dub rest)
...
local function buildEffect(): typeof(state)
	...
	-- Four gradient frames radiating inward from each edge, tinted red.
	local vigFrames = {}
	local vigData = {
...
-- Tears down every resource. `animated` = tween out first; false = cut
-- instantly (used on respawn so nothing carries over to the new life).
...
-- Runs the repeating lub-dub heartbeat loop. Keyed off object identity so a
-- token refresh (duration extension) never restarts the loop — it keeps
-- running uninterrupted while only the end-timer is bumped.
...
-- Hard-stops everything immediately — used on respawn so no red tint or
-- vignette bleeds into the new life.
